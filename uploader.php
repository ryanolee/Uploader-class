...
class upload{
	...
	/**
	 * Gets html for the upload.
	 *
	 * @param boolean $to_return To return the html as a string or directly echo it out
	 * @return void|string
	 */
	public function get_html_segment($to_return=false){//@todo DO NOT FORGET TO POST FILE_NO SO THAT FILE CAN BE IDENTIFIED WHEN IT IS RETURNED TO THE SERVER;
		$data="";
		if ($this->upload_id==1){//if this is the first block of html that is looking to be returned.
			...
			$(document).ready(function() {
				...
				function handle_file_button_click(upload_button,called) {
						...
						console.log('Beginning ajax file upload.')
		...
		?>
		<div id='form_wrapper'>
			<form action='<?php echo $this->upload_endpoint;?>' method='post' enctype='multipart/form-data' id='http_upload_form'>
				<div class='row'>
					<div class='six columns'>
						<input file_no='<?php echo(string)$this->upload_id;?>' type='file' id='http_upload' name='http_upload' accept='<?php echo implode(",",$this->accepted_datatypes)?>'>
					</div>
					<!--hidden input for non html5 functionality-->
					<input type='hidden' name='file_no' value='<?php echo (string)$this->upload_id; ?>' />
					<input type='hidden' name='csrf_token' value='<?php echo $this->csrf_token;?>'>
					<div class='six columns'>
						<button id='http_upload_button' upload_button_id='<?php echo(string)$this->upload_id;?>'>
							Click here to upload!
						</button>
					</div>
				</div>
				<div class='row'>
					<div class='twelve columns' id='input_validate_area' style='background-color:#ff4d4d;'></div>
				</div>
			</form>
			<div id='post_upload_area' class='row' style='display:none'>
				<div class='ten columns'>
					<progress value='0' max='100' style='width:100%;height:40px;'></progress>
				</div>
				<div class='two columns'>
					<button style='background-color:#ff3333' id='abort_upload_button'><font color='white'>X</font></button>
				</div>
			</div>
		</div>
		<?php
	...
	if(!isset($_POST["js_enabled"])){//if js not enabled
		...
		<html>
			...
			<body>
				...
				<?php
				echo (isset($_SERVER["HTTP_REFERER"])?"<a href='".$_SERVER["HTTP_REFERER"]."'>click here</a> to go back.":"")
				?>
			</body>
		</html>
		<?php
